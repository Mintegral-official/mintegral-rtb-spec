# Mintegral RTB Specification ver 2.5 for Supply side
...
* [1. Request specification](#1-request-specification)
...
* [2. Response specification](#2-response-specification)
  ...
  * [c. price & Macro substitution](#c-macro-substitution)
  * [d. tracking](#d-tracking)
* [3. Code table](#3-code-table)
* [4. Bid Request/Response Examples](#4-bid-requestresponse-examples)
...
Mintegral会提供流量入口的Endpoint，这个EndPoint应该是一个HTTP或者HTTPS的链接地址，支持分地区提供不同的EndPoint.
...
| 字段 | 类型    | 描述       |
|----------|-------- |---------------------------|
| oaidmd5 | string | md5(oaid)
| imei           | string | 硬件设备 ID，安卓传 IMEI  |
| android_id     | string  | 设备平台 ID，安卓传 Android ID  |
| idfamd5            | string  | md5(IDFA) |
| idfasha1            | string  | sha1(IDFA) |
| ifv            | string  | IDFV |
...
| 广告类型 | 上报类型  |  位置  | 
|-----|-----|----|
| Banner | Impression | 返回的html代码块中, 以及bid.ext.imptrackers|
